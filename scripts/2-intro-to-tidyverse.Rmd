...
```
Note: this dataset mostly tracks the population that is 10 or older, and not every country is included in the dataset (in fact many aren't), so this is only the population included in this dataset (not the actually world pop).  

We can find the female population of our dataset using `filter()` and `summarise()`:
```{r}
literacy %>% 
  filter(sex=="Female") %>% 
  summarise(female_pop = sum(total))
```
Don't forget that you can use R as a calculator!
```{r}
1396874044/2792717233
...
```{r}
# What is the total (10+) population for each country? 
literacy %>% 
