...
library(janitor) 
library(googlesheets4) # import from a google sheet
```
...
To illustrate some of these options, we'll use a file from a city website on vendor payments:
...
```{r}
gs_data <- read_sheet("14riMfTuuy8s1Uk6FEEVUXQicgeQOKve5ipEDOY_m1KI")
```
